...
function prepareLineChartData(data, continents, colors) {
  ...
  const reduceTotalCasesEurope = (values) =>
    d3.sum(values, (leaf) => leaf.total_cases);
  const totalCasesMapEurope = d3.rollup(
    data[1],
    reduceTotalCasesEurope,
    groupByEurope
  );
  const totalCasesEurope = Array.from(totalCasesMapEurope).sort(
    (a, b) => a[0] - b[0]
  );
  const datesEurope = totalCasesEurope.map((d) => d[0]);

  const groupByAfrica = (d) => d.date;
  const reduceTotalCasesAfrica = (values) =>
    d3.sum(values, (leaf) => leaf.total_cases);
  const totalCasesMapAfrica = d3.rollup(
    data[2],
    reduceTotalCasesAfrica,
    groupByAfrica
  );
  const totalCasesAfrica = Array.from(totalCasesMapAfrica).sort(
    (a, b) => a[0] - b[0]
  );
  const datesAfrica = totalCasesAfrica.map((d) => d[0]);

  const groupByNorthAmerica = (d) => d.date;
  const reduceTotalCasesNorthAmerica = (values) =>
    d3.sum(values, (leaf) => leaf.total_cases);
  const totalCasesMapNorthAmerica = d3.rollup(
    data[3],
    reduceTotalCasesNorthAmerica,
    groupByNorthAmerica
  );
  const totalCasesNorthAmerica = Array.from(totalCasesMapNorthAmerica).sort(
    (a, b) => a[0] - b[0]
  );
  const datesNAmerica = totalCasesNorthAmerica.map((d) => d[0]);

  const groupBySAmerica = (d) => d.date;
  const reduceTotalCasesSAmerica = (values) =>
    d3.sum(values, (leaf) => leaf.total_cases);
  const totalCasesMapSAmerica = d3.rollup(
    data[4],
    reduceTotalCasesSAmerica,
    groupBySAmerica
  );
  const totalCasesSAmerica = Array.from(totalCasesMapSAmerica).sort(
    (a, b) => a[0] - b[0]
  );
  const datesSAmerica = totalCasesSAmerica.map((d) => d[0]);

  const groupByOceania = (d) => d.date;
  const reduceTotalCasesOceania = (values) =>
    d3.sum(values, (leaf) => leaf.total_cases);
  const totalCasesMapOceania = d3.rollup(
    data[5],
    reduceTotalCasesOceania,
    groupByOceania
  );
  const totalCasesOceania = Array.from(totalCasesMapOceania).sort(
    (a, b) => a[0] - b[0]
  );
  const datesOceania = totalCasesOceania.map((d) => d[0]);

  const dates = datesAsia.concat(
    datesEurope,
    datesAfrica,
    datesNAmerica,
    datesSAmerica,
  ...
  const lineData = {
    series: [
      ...
      },
      {
        ...
        lblClass: `namerica`,
        color: `${colors[3]}`,
        values: totalCasesNorthAmerica.map((d) => ({
          date: d[0],
          value: d[1],
        })),
        lblPosition: [4, 24],
...
function prepareScatterPlotData(data) {
  ...
  const minMaxGDP = asiaGDPTotal.concat(
    europeGDPTotal,
    africaGDPTotal,
    nAmericaGDPTotal,
    sAmericaGDPTotal,
  ...
  const lineData = {
    series: [
      {
        total_cases_per_million: asiaTop5.map(
          (d) => d[1].total_cases_per_million
        ),
        objects: asiaObjects,
        lblPosition: [4, -20],
      ...
      {
        total_cases_per_million: europeTop5.map(
          (d) => d[1].total_cases_per_million
        ),
      ...
      {
        total_cases_per_million: africaTop5.map(
          (d) => d[1].total_cases_per_million
        ),
      ...
      {
        total_cases_per_million: nAmericaTop5.map(
          (d) => d[1].total_cases_per_million
        ),
      ...
      {
        total_cases_per_million: sAmericaTop5.map(
          (d) => d[1].total_cases_per_million
        ),
      ...
      {
        total_cases_per_million: oceaniaTop5.map(
          (d) => d[1].total_cases_per_million
        ),
...
function prepareLineChartDataVacsCases(data, country, colors) {
  ...
  const yValuesMax = [values_cases[values_cases.length - 1]];
  const yValuesMin = [values_cases[0]];
  const yMax = d3.max(yValuesMax);
  const yMin = d3.min(yValuesMin);
  // // const yMax2 = yValuesMax.sort((a, b) => a - b) - testing max
  // // const yMin2 = yValuesMin.sort((a, b) => a - b) - testing min
...
function prepareLineChartDataVacs(data, country, colors) {
  ...
  const values_vaccinations = total_vaccinations
    .map((d) => d[1])
    .sort((a, b) => a - b);
  ...
  const hasTotalDeaths =
    total_deaths.length > 0 && total_deaths[total_deaths.length - 1][1] > 0;

  // Check if the last value in total_vaccinations is greater than 0
  const hasTotalVaccinations =
    total_vaccinations.length > 0 &&
    total_vaccinations[total_vaccinations.length - 1][1] > 0;

  // Check if the last value in total_hospital is greater than 0
  const hasTotalHospital =
    total_hospital.length > 0 &&
    total_hospital[total_hospital.length - 1][1] > 0;
